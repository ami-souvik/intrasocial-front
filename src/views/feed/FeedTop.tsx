...
export default function FeedTop() {
  ...
  return (
    <Body className="fixed w-screen top border-b border-slate-700 z-50 backdrop-blur-[5px] py-3">
      <Left />
      <Mid>
        <div className="flex justify-end items-center relative">
          ...
          <input
            className="p-2 w-full bg-transparent border border-slate-600"
            placeholder="Search me"
            value={search}
            onChange={(e) => setSearch(e.target.value)}
          ></input>
          <button
            className="flex bg-teal-700 mx-2 space-x-2"
            onClick={() => window.open('/content/create', '_self')}
          >
            <CiEdit size={22} />
            <p>Write</p>
          </button>
